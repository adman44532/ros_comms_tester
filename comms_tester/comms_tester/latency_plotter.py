...
def read_csv_data(csv_file_path):
    ...
    for row in reader:
        try:
            ...
            logger.error(f"Failed to convert data row: {row}")  # Most likely a packet loss
            continue
...
def compute_test_duration(system_info, data):
    ...
    last_entry = data[-1]
    end_time = last_entry['receive_time'] if last_entry['receive_time'] else last_entry['send_time']
    end_dt = start_dt + timedelta(seconds=end_time - data[0]['send_time'])  # Calculate relative to start
...
def main():
    ...
    for csv_file in csv_files:
        ...
        if data:
            ...
            output_path = os.path.join(data_folder, f"{os.path.splitext(csv_file)[0]}.png")
